...
app.get('/teams/:id/players', async (req, res) => {
  ...
  const data = await rapidapi.players.getPlayersByTeam(req.params.id, season);
  if (data.error) {
    res.status(400);
  } else {
    res.json(data);
...
app.get('players/search/:name', async (req,res) => {
  const data = await rapidapi.players.getPlayerBySearch(req.params.name);
  if (data.error) {
    res.status(400);
  } else {
    res.json(data);
